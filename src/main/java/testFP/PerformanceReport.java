...
public class PerformanceReport {
    ...
    public void display() {
        ...
        System.out.printf("%10s %17s %19s %22s %18s %12s\n", "Rank", "Employee ID", "Employee Name", "Total Sales (RM)", "Transactions", "Outlet");
        
        for (int i = 0; i < performances.length; i++) {
            EmployeePerformance employee = performances[i];
            System.out.printf("%10s %17s %19s %22.2f %18d %12s\n", (i + 1), employee.getEmployeeID(), employee.getEmployeeName(), employee.getTotalSales(), employee.getTransactionCount(), employee.getOutlet());
        }
    }
}
